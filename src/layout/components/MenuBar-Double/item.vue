<template>
  <el-menu-item class="submenu" v-if="!routeChildren && !meta.hidden" :index="resolvePath(menu.path)">
    <i v-if="meta.icon" :class="meta.icon"></i>
    <span>{{meta.title || route.name}}</span>
  </el-menu-item>
  <el-sub-menu class="submenu" v-else-if="!meta.hidden" :index="resolvePath(route.path)">
    <template #title>
      <i v-if="meta.icon" :class="meta.icon"></i>
